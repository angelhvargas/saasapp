...
from pinax.stripe.actions.sources import (
...
from saas_app.payments.plan import get_plans
...
class OctobatForm(forms.Form):
    ...
    def clean_transactionDetails(self):
        ...
        try:
            ...
            logger.error(
                "Unable to clean transactionDetails for data: {}".format(details),
                exc_info=True,
            )
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            cus = Customer.objects.create(
                user=self.request.user, stripe_id=stripe_customer["id"]
            )
            ...
            messages.error(
                self.request,
                "Unable to communicate with stripe. Please try again later",
            )
            logger.error(
                "Stripe Error during checkout for customer {}".format(
                    self.request.user
                ),
                exc_info=True,
            )
