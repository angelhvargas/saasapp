...
class CheckoutViewTestCase(UserTestCase):
    ...
    # @patch("saas_app.payments.views.sync_customer")
    # @patch("saas_app.payments.views.StripeCustomer")
...
class SubscriptionTestCase(UserTestCase):
    def setUp(self):
        ...
        self.sub = Subscription.objects.create(
            customer=self.cust, quantity=1, plan=self.plan, start=timezone.now()
        )
...
class PaymentMethodCreateViewTestCase(UserTestCase):
    ...
    def test_post_on_error(self, create_card):
        ...
        response = self.client.post(reverse("pinax_stripe_payment_method_create"), {})
