...
from saas_app.beta.models import Invite, Request
from saas_app.beta.admin import send_invite, cancel_invite


class SendInviteTestCase(TestCase):
    @patch("saas_app.beta.admin.send_mail")
    def test_invite(self, send_mail_mock):
        r = Request.objects.create(email="testuser@example.com")

        send_invite(modeladmin=None, request=Mock(), queryset=[r])
...
class CancelInviteTestCase(TestCase):
    def test_cancel_invite(self):
        ...
        cancel_invite(modeladmin=None, request=Mock(), queryset=[req])
